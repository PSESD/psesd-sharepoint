...
PsesdNewsfeed.prototype.renderList = function($list, resource, isSticky) {
  ...
  jQuery.each(resource.items, function(index, item) {
    if (!item.$element) {
      ...
      item.elements.$title = $("<a />", {'href': item.OriginalPath}).addClass('psesd-timeline-item-light psesd-timeline-item-title').appendTo(item.$element);
	  item.elements.$titleContent = $("<span />", {'title': item.Title}).html(item.Title + ' ').addClass('psesd-timeline-item-title-content').appendTo(item.elements.$title);

	  if (item.AnnouncementUrl) {
		item.$element.toggleClass('collapsed');
        item.elements.$title.attr('href', item.AnnouncementUrl).attr('target', '_blank');
        item.elements.$titleContent.append($("<i />", {'class': 'fa fa-external-link'}));
		item.elements.$expand = $('<span />', {'class': 'psesd-timeline-item-expandable'}).html('Click to Open').prependTo(item.elements.$title);
      } else {
        if (!item.BodyOWSMTXT || item.BodyOWSMTXT === '') {
          item.elements.$title.click(function() {
			...
			item.elements.$contentContainer = $("<div />", {}).addClass('psesd-timeline-item-content').appendTo(item.$element);
			item.elements.$content = $("<div />", {}).html(item.BodyOWSMTXT).addClass('psesd-timeline-item-content-area').appendTo(item.elements.$contentContainer);
			item.$element.addClass('collapsed');
			item.elements.$expand = $('<span />', {'class': 'psesd-timeline-item-expandable'}).html('Click to Read').prependTo(item.elements.$title);
	    	item.elements.$title.click(function() {
				item.$element.toggleClass('collapsed');
				return false;
	        });
			item.elements.$title.hover(
				function() {
      ...
      item.elements.$datePosted = $("<span />", {'title': 'Date Posted'}).html(date.toLocaleTimeString("en-US", dateOptions)).addClass('psesd-timeline-item-date').prependTo(item.elements.$title);
    }
